...
- **开发环境**: 在开发模式下自动启用
- **生产环境**: 在 URL 后添加 `?debug=true` 参数启用，例如：`https://your-domain.com/?debug=true`
- **功能**: 提供控制台、元素检查、网络监控、资源管理、设备信息等调试功能

### 如何使用
1. 在浏览器中打开应用
2. 如在生产环境，添加 `?debug=true` 参数到 URL
3. 页面右下角会出现 Eruda 调试按钮
4. 点击按钮打开调试面板
...
4. 前后端需要分别启动才能正常工作
5. 使用 Eruda 调试工具时注意不要在生产环境中长期启用，以免暴露敏感信息
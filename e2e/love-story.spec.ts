...
test('both members edit shared memories and photos; anonymous access and reminders obey real handlers', async ({
  ...
  try {
    ...
    await page.getByRole('button', { name: '写回忆' }).click()
    ...
    await expect(partner.getByRole('button', { name: '改回私密' })).toBeVisible()
    ...
    await expect(partner.getByRole('button', { name: '公开', exact: true })).toBeVisible()
    ...
    await page.getByRole('button', { name: '记纪念日' }).click()
    ...
    await partner.goto(`${origin}/app/memories`)
    await partner.getByRole('button', { name: '写回忆' }).click()
    ...
    await partner.goto(`${origin}/app/anniversaries`)
    await partner.getByRole('button', { name: '记纪念日' }).click()

...
export interface MemoryStore extends AuthStore, StoryStore, MediaStore, ReminderStore {
  readonly state: {
    spaces: Space[]
    users: UserRecord[]
...
export function createMemoryStore(): MemoryStore {
  ...
  return {
    ...
    async findSession(tokenHash, now) {
      ...
      return space ? ({ user, space } satisfies SessionView) : null
    },
    ...
    async createInvitation(input) {
      state.invitations.splice(
        ...
        ...state.invitations.filter(
          (candidate) => !(candidate.spaceId === input.spaceId && candidate.email === input.email)
        )
      )
    ...
    async claimDelivery(key) {
      ...
      state.deliveries.push({ ...key, status: 'sending' })
      return true
    },
    async finishDelivery(key, result) {
      ...
      if (delivery) delivery.status = result.error ? 'failed' : 'sent'
    }
  }
}
